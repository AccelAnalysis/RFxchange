...
function activityTrigger(document: string) {
  return onDocumentCreated({ document, region: RFXCHANGE_FUNCTIONS_REGION, retry: true }, async event => {
    const data = event.data?.data();
    if (typeof data?.actorUserId !== "string" || typeof data.occurredAt !== "string" || !Number.isFinite(Date.parse(data.occurredAt))) return;
    const ref = getFunctionsFirestore().collection("lifecycleEnrollments").doc(data.actorUserId);
    ...
    await getFunctionsFirestore().runTransaction(async tx => {
      ...
      tx.set(ref, { userId: data.actorUserId, lastActivityAt: occurredAt, lastActivityEventId: event.id, nextEvaluationAt: new Date().toISOString() }, { merge: true });
    });
  });
}
