...
type OutcomeDraft = Readonly<{
  status: RfxExecutionOutcome["status"];
  executionNote: string;
  outcomeSummary: string;
  outcomeValue: string;
}>;

function factorDraft(review?: RfxEvaluationFactorReview): FactorDraft {
  return Object.freeze({
    gate: review?.gate ?? "not-reviewed",
    score: review?.scoreBasisPoints === null || review?.scoreBasisPoints === undefined
      ? ""
      : String(review.scoreBasisPoints / 100),
    note: review?.note ?? "",
  });
}

function initialEvaluationDraft(
  workspace: RfxIssuerWorkspace,
  ...
  return Object.fromEntries(workspace.responses.map((response) => {
    ...
    const ownReview = evaluation?.reviews.find(
      (item) => String(item.evaluatorMembershipId) === viewerMembershipId,
    );
    return [response.id, Object.freeze({
      factors: Object.freeze(Object.fromEntries(definitions.map((factor) => [
        factor.id,
        factorDraft(ownReview?.factors.find((item) => item.factorId === factor.id)),
      ]))),
...
function responseValue(item: RfxResponse["items"][number]): string {
  ...
  if (item.format === "pricing") {
    return item.priceMinor === null || !item.currency
      ? "Not provided"
      : `${item.currency} ${(item.priceMinor / 100).toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`;
  }
  if (item.format === "attachment") {
    return item.attachmentAssetIds.length
      ? `${item.attachmentAssetIds.length} attachment${item.attachmentAssetIds.length === 1 ? "" : "s"}`
      : "No attachment";
  }
...
export function RfxIssuerEvaluationWorkspace({
  initialWorkspace,
  viewerMembershipId,
  returnHref,
}: Readonly<{
  ...
  const [evaluationDrafts, setEvaluationDrafts] = useState<Record<string, EvaluationDraft>>(
    () => initialEvaluationDraft(initialWorkspace, viewerMembershipId),
  );
  const [outcomeDrafts, setOutcomeDrafts] = useState<Record<string, OutcomeDraft>>(
    () => initialOutcomeDraft(initialWorkspace),
  );
  ...
  const definitions = [
    ...(workspace.snapshot.aggregate.definition?.evaluationDefinition.factors ?? []),
  ].sort((left, right) => left.order - right.order);
  const title = workspace.snapshot.aggregate.package?.title
    ?? workspace.snapshot.aggregate.requestFamily.labelSnapshot;
  const selectedResponseId = workspace.evaluations.find(
    (item) => item.decision === "selected",
  )?.responseId ?? null;

  async function refresh() {
    const result = await fetch(
      `/api/rfx-cycle?mode=issuer&rfxId=${encodeURIComponent(String(workspace.snapshot.rfxId))}`,
  ...
  function updateFactor(responseId: string, factorId: string, patch: Partial<FactorDraft>) {
    setEvaluationDrafts((current) => {
      const responseDraft = current[responseId];
      const currentFactor = responseDraft?.factors[factorId];
      if (!responseDraft || !currentFactor) return current;
      return {
        ...current,
        [responseId]: Object.freeze({
          ...responseDraft,
          factors: Object.freeze({
            ...responseDraft.factors,
            [factorId]: Object.freeze({ ...currentFactor, ...patch }),
          }),
        }),
      };
    });
  ...
  function updateEvaluationDraft(responseId: string, patch: Partial<EvaluationDraft>) {
    setEvaluationDrafts((current) => {
      const responseDraft = current[responseId];
      if (!responseDraft) return current;
      return { ...current, [responseId]: Object.freeze({ ...responseDraft, ...patch }) };
    });
  }
  ...
  async function answerQuestion(questionId: string) {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "The answer could not be saved.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function issueAddendum() {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "The addendum could not be issued.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function saveEvaluation(responseId: string) {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "The evaluator review could not be saved.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function decide(responseId: string, decision: "selected" | "not-selected") {
    ...
    try {
      ...
      setNotice(
        decision === "selected"
          ? "Response selected and connection/execution record opened."
          : "Response marked not selected.",
      );
    } catch (error) {
  ...
  function updateOutcomeDraft(outcomeId: string, patch: Partial<OutcomeDraft>) {
    setOutcomeDrafts((current) => {
      ...
      return { ...current, [outcomeId]: Object.freeze({ ...outcome, ...patch }) };
    });
  }
    ...
    try {
      ...
      setNotice(
        draft.status === "completed"
          ? "Outcome completed and emitted into RFxchange Intelligence input."
          : "Execution status updated.",
      );
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "The execution update could not be saved.");
    } finally {
      setBusy(null);
    }
  }
  ...
  return <ParticipantShell activeItem="opportunities-rfx">
    <main className={styles.workspace} data-rfx-issuer-evaluation-workspace>
      ...
      <section className={styles.section}>
        <h2>Addenda</h2>
        {workspace.addenda.length ? <ul className={styles.addenda}>{workspace.addenda.map((addendum) => <li key={addendum.id}>
          <strong>{addendum.title}</strong><p>{addendum.body}</p><span>{addendum.requiresAcknowledgment ? "Acknowledgment required" : "Informational"}</span>
        ...
        {!workspace.responses.length ? <div className={styles.notice}>No submitted responses yet.</div> : workspace.responses.map((response, responseIndex) => {
          ...
          return <article className={styles.response} key={response.id} data-submitted-response={response.id}>
            ...
            <div className={styles.meta}>
              <span>Submitted {receipt?.submittedAt ? new Date(receipt.submittedAt).toLocaleString() : response.submittedAt ? new Date(response.submittedAt).toLocaleString() : "—"}</span>
              <span>{response.collaboratorOrganizationIds.length} teammate{response.collaboratorOrganizationIds.length === 1 ? "" : "s"}</span>
              <span>{evaluation?.decision.replaceAll("-", " ") ?? "not evaluated"}</span>
            </div>
            <ul className={styles.responseItems}>{response.items.map((item) => <li key={item.sectionId}>
              <strong>{item.titleSnapshot}</strong><p>{responseValue(item)}</p>
              {item.attachmentAssetIds.length ? <div className={styles.actions}>{item.attachmentAssetIds.map((assetId, index) => <a key={assetId} href={`/api/rfx-cycle/attachment/${encodeURIComponent(assetId)}`}>Open attachment {index + 1}</a>)}</div> : null}
            </li>)}</ul>
            ...
            {workspace.canEvaluate && draft ? <section className={styles.card}>
              ...
              {definitions.map((factor) => {
                ...
                return <div className={styles.factor} key={factor.id} data-evaluation-factor={factor.id}>
                  ...
                  <div className={styles.meta}>
                    <span>{factor.treatment.replaceAll("-", " ")}</span>
                    {factor.weightBasisPoints !== null ? <span>Weight {factor.weightBasisPoints / 100}%</span> : null}
                  </div>
                  {requiresGate(factor) ? <label>Gate<select value={value.gate} onChange={(event) => updateFactor(response.id, factor.id, { gate: event.target.value as FactorDraft["gate"] })}>
                    <option value="not-reviewed">Not reviewed</option><option value="pass">Pass</option><option value="fail">Fail</option>
                  </select></label> : null}
                  ...
                  {consensus ? <div className={styles.consensus}>
                    <span>Consensus gate: {consensus.gate}</span>
                    {consensus.averageScoreBasisPoints !== null ? <span>Average: {consensus.averageScoreBasisPoints / 100}</span> : null}
                    <span>{consensus.reviewCount} review{consensus.reviewCount === 1 ? "" : "s"}</span>
                  </div> : null}
                </div>;
              })}
              ...
              <div className={styles.actions}><button type="button" disabled={busy !== null || (evaluation !== undefined && evaluation.decision !== "under-review")} onClick={() => void saveEvaluation(response.id)}>Save evaluator review</button></div>
            </section> : null}

            {workspace.canEvaluate && evaluation?.decision === "under-review" && draft ? <section className={styles.card}>
              ...
              <div className={styles.actions}>
                <button className={styles.secondary} type="button" disabled={busy !== null || !draft.consensusNote.trim()} onClick={() => void decide(response.id, "not-selected")}>Not selected</button>
                <button className={styles.primary} type="button" disabled={busy !== null || Boolean(selectedResponseId && selectedResponseId !== response.id) || !draft.consensusNote.trim() || !draft.connectionNote.trim()} onClick={() => void decide(response.id, "selected")}>Select & connect</button>
              </div>
            </section> : null}
            ...
            {evaluation?.decision === "selected" && outcome ? <section className={styles.outcome}>
              ...
              {workspace.canEvaluate && outcomeDrafts[outcome.id] ? <>
                <label>Status<select value={outcomeDrafts[outcome.id].status} onChange={(event) => updateOutcomeDraft(outcome.id, { status: event.target.value as OutcomeDraft["status"] })}>
                  <option value="connected">Connected</option><option value="executing">Executing</option><option value="completed">Completed</option>
                </select></label>
