...
export function RFxDraftWorkspace({
  ...
  const [requestFamilyId, setRequestFamilyId] = useState(
    requestFamilies[0]?.id ?? "",
  );
  ...
  const selectedFamily =
    requestFamilies.find((family) => family.id === requestFamilyId) ?? null;
  ...
  async function post(body: Record<string, unknown>) {
    ...
    if (!response.ok)
      throw new Error(
        String(payload.detail ?? payload.error ?? t("rfxWorkspace.error")),
      );
  ...
  async function createDraft() {
    ...
    try {
      ...
      setDrafts((current) => [
        result.aggregate,
        ...current.filter((draft) => draft.id !== result.aggregate.id),
      ]);
  ...
  async function changeFamily(nextFamilyId: string) {
    if (
      !selectedDraft ||
      nextFamilyId === selectedDraft.requestFamily.requestFamilyId
    )
      return;
    setBusy(true);
    setError(null);
    ...
    try {
      ...
      setDrafts((current) =>
        current.map((draft) =>
          draft.id === result.aggregate.id ? result.aggregate : draft,
        ),
      );
      clearRetryStableCommand({
        storage: commandStorage,
        ...
        <div data-rfx-workspace="private-drafts">
          ...
          {!canCreate ? (
            ...
            <div className={styles.layout}>
              <aside
                className={styles.draftRail}
                aria-label={t("rfxWorkspace.draftsLabel")}
              >
                    ...
                    {drafts.map((draft) => (
                      <li key={draft.id}>
                        <button
                          type="button"
                          aria-current={
                            selectedDraft?.id === draft.id ? "true" : undefined
                          }
                ...
                {selectedDraft ? (
                  <>
                    ...
                    <h2 id="rfx-task-title">
                      {selectedDraft.package?.title ||
                        selectedDraft.requestFamily.labelSnapshot}
                    </h2>
                    <p>{selectedDraft.requestFamily.purposeSnapshot}</p>
                    <dl className={styles.facts}>
                      <div>
                        <dt>{t("rfxWorkspace.lifecycle")}</dt>
                        <dd>
                          {selectedDraft.requestFamily.lifecycleSnapshot
                            .map(readableLifecycle)
                            .join(" → ")}
                        </dd>
                      </div>
                      <div>
                        <dt>{t("rfxWorkspace.release")}</dt>
                        <dd>
                          AMACS{" "}
                          {selectedDraft.requestFamily.amacsReleaseVersion}
                        </dd>
                      </div>
                      <div>
                        <dt>{t("rfxWorkspace.versionLabel")}</dt>
                  ...
                  <>
                    ...
                    <fieldset
                      ...
                      disabled={busy || requestFamilies.length === 0}
                    >
                      ...
                      {requestFamilies.map((family) => (
                        <label key={family.id}>
                          <input
                            ...
                            name="request-family"
                            value={family.id}
                            checked={requestFamilyId === family.id}
                            onChange={() => setRequestFamilyId(family.id)}
                          />
                          <span>
                    ...
                    {selectedFamily ? (
                      <p className={styles.lifecyclePreview}>
                        {selectedFamily.lifecycle
                          .map(readableLifecycle)
                          .join(" → ")}
                      </p>
                    ) : null}
                    <button
                      ...
                      type="button"
                      disabled={busy || !requestFamilyId}
                      onClick={() => void createDraft()}
                    >
                      {busy
                        ? t("rfxWorkspace.creating")
                        : t("rfxWorkspace.createAction")}
                    </button>
                  </>
                )}
              </section>
