...
function responseDeadlineOpen(aggregate: RfxAggregate, now: string): boolean {
  const deadline = aggregate.package?.timing.responseDeadline;
  if (!deadline) return false;
  const deadlineTime = Date.parse(`${deadline}T23:59:59.999Z`);
  const currentTime = Date.parse(now);
  return Number.isFinite(deadlineTime) && Number.isFinite(currentTime) && deadlineTime > currentTime;
}
...
export class Wave4GapPublicationRepository implements RfxRepository {
  ...
  async publish(
    ...
    return this.db.runTransaction(async (transaction) => {
      ...
      const account = await this.accountSecurity.inspect(bundle.authentication.subject);
      if (
        account.provider !== bundle.authentication.provider ||
        account.subject !== bundle.authentication.subject ||
        authenticationAccountState(account) !== "active" ||
        authenticationCredentialState(account, bundle.authentication.authenticatedAt) !== "current"
      ) {
        throw new RfxPersistenceConflictError("RFx publication account authority changed.");
      }
      ...
      if (!responseDeadlineOpen(current, this.now())) {
        throw new RfxPersistenceConflictError("RFx publication deadline is no longer open.");
      }
