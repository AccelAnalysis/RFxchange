...
const NOTICE_COPY = Object.freeze({
  "en-US": Object.freeze({ consentRequired: "Review and acknowledge the exact recipient and shared information before sending.", requestSentPrefix: "Request sent to", requestFailed: "Request failed.", resourceActionFailed: "Resource action failed.", providerActionFailed: "Provider request action failed.", yourOrganization: "Your organization", searchMatch: "Matches this search and locality", localityMatch: "Serves this locality" }),
  es: Object.freeze({ consentRequired: "Revisa y confirma el destinatario exacto y la información compartida antes de enviar.", requestSentPrefix: "Solicitud enviada a", requestFailed: "La solicitud falló.", resourceActionFailed: "La acción del recurso falló.", providerActionFailed: "La acción de la solicitud al proveedor falló.", yourOrganization: "Tu organización", searchMatch: "Coincide con esta búsqueda y localidad", localityMatch: "Presta servicio en esta localidad" }),
  fr: Object.freeze({ consentRequired: "Vérifiez et confirmez le destinataire exact et les informations partagées avant l’envoi.", requestSentPrefix: "Demande envoyée à", requestFailed: "Échec de la demande.", resourceActionFailed: "Échec de l’action sur la ressource.", providerActionFailed: "Échec de l’action sur la demande fournisseur.", yourOrganization: "Votre organisation", searchMatch: "Correspond à cette recherche et à cette localité", localityMatch: "Dessert cette localité" }),
  de: Object.freeze({ consentRequired: "Prüfen und bestätigen Sie vor dem Senden den genauen Empfänger und die freigegebenen Informationen.", requestSentPrefix: "Anfrage gesendet an", requestFailed: "Anfrage fehlgeschlagen.", resourceActionFailed: "Ressourcenaktion fehlgeschlagen.", providerActionFailed: "Anbieteranfrageaktion fehlgeschlagen.", yourOrganization: "Ihre Organisation", searchMatch: "Entspricht dieser Suche und Region", localityMatch: "Bedient diese Region" }),
  it: Object.freeze({ consentRequired: "Controlla e conferma il destinatario esatto e le informazioni condivise prima dell’invio.", requestSentPrefix: "Richiesta inviata a", requestFailed: "Richiesta non riuscita.", resourceActionFailed: "Azione sulla risorsa non riuscita.", providerActionFailed: "Azione sulla richiesta al fornitore non riuscita.", yourOrganization: "La tua organizzazione", searchMatch: "Corrisponde a questa ricerca e località", localityMatch: "Serve questa località" }),
});
...
function providerHref(id: string, context?: ResourcesMobileNavigationContext) {
  return resourcesHref({ organization: id, provider: id }, context);
}
...
function actionProjection(definition: ReturnType<typeof exchangeRoomActionDefinitionsForLens>[number], input: ResourcesMobileProjectionInput, selection: ExchangeSelectionState): ExchangeRoomActionProjection {
  ...
  const href = definition.id === "resources.offer-request"
    ...
    : definition.id === "resources.manage-view" && selectedResource ? resourcesHref({ provider: selectedOrganizationId, resource: selection.selectedRecord!.recordId }, input.navigationContext) : null;
