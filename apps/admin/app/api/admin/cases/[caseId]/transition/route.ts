...
import { resolveAdminPortalAccess, resolveAdminRoute } from "@/src/infrastructure/auth/admin-route-runtime";
import {
...
export async function POST(
  ...
  try {
    // Authenticate before looking up a case so its existence cannot be probed anonymously.
    // The case-specific write permission and scope are still revalidated below.
    const cookieStore = await cookies();
    const sessionCookie = cookieStore.get(RFXCHANGE_SESSION_COOKIE_NAME)?.value;
    const portalAccess = await resolveAdminPortalAccess({ sessionCookie });
    if (portalAccess.kind !== "authorized") {
      return NextResponse.json(
        { error: "This administrative case action is unavailable." },
        { status: portalAccess.kind === "unauthenticated" ? 401 : 403 },
      );
    }
