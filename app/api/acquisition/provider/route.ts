...
export async function GET(request: NextRequest) {
  ...
  if (!parseAcquisitionContextToken(serializedToken)) return NextResponse.redirect(new URL("/join?entry=provider&status=unavailable", request.url), 303);
  const response = NextResponse.redirect(new URL("/join?entry=provider", request.url), 303);
