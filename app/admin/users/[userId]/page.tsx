...
import { authorizeScopedAdministrativeAction, createScopedAdministrativeActionRequirement } from "@/src/domain/admin-authorization/grants";
import { scopeUserAccess360OrganizationData } from "@/src/application/admin/user-access-360";
...
export default async function Page({params,searchParams}:Readonly<{params:Promise<{userId:string}>;searchParams?:Promise<Readonly<Record<string,string|string[]|undefined>>>}>){
  const query=searchParams?await searchParams:{}; const requestedScope=typeof query.scope==="string"?query.scope:null;
  ...
  const access=await resolveAdminPortalAccess({sessionCookie}); const returnTo=encodeURIComponent(`/admin/users/${encodeURIComponent(userId)}${requestedScope?`?scope=${encodeURIComponent(requestedScope)}`:""}`);
  ...
  const destinations=access.destinations.filter((candidate)=>candidate.key==="users-access"&&(!requestedScope||candidate.scope.value===requestedScope)&&["user.profile.read","user.access.read"].every((permission)=>authorizeScopedAdministrativeAction(access.authority,access.grants,createScopedAdministrativeActionRequirement({permission,access:"read",scope:candidate.scope.value}),{now,satisfiedConditionKeys:Object.freeze([])}).kind==="allow")); if(!destinations.length)notFound();
  ...
  const scoped=scopeUserAccess360OrganizationData({memberships,organizationAuthorizations:authorizations},allowed.scope);
  const profiles=(await Promise.all(scoped.memberships.map(async(membership)=>{const snapshot=await db.collection("organizationProfiles").where("organizationId","==",membership.organizationId).limit(1).get();const id=snapshot.docs[0]?.id;return id?profilesRepo.getById(id as OrganizationProfileId):null;}))).filter((value):value is NonNullable<typeof value>=>value!==null);
  const projection=buildUserAccess360(access.authority,{user,...scoped,organizationProfiles:profiles,lastLoginAt:null});
  return <AdminPortalShell destinations={access.destinations} currentDestination="users-access" currentScope={allowed.scope.value}>
    <main className={styles.workspace}><header className={styles.header}><div><p className={styles.eyebrow}>Network & Identity</p><h1>{projection.identity.name}</h1><p className={styles.description}>User & Access 360 · authoritative identity, organization membership and effective access.</p></div><Link className={styles.secondaryAction} href={`/admin/users?scope=${encodeURIComponent(allowed.scope.value)}`}>Back to users</Link></header>
      ...
      <aside className={styles.inspector}><p className={styles.eyebrow}>Access context</p><h2>{projection.identity.id}</h2><dl className={styles.facts}><div><dt>Memberships</dt><dd>{projection.memberships.length}</dd></div><div><dt>Granular permissions</dt><dd>{projection.granularPermissions.length}</dd></div><div><dt>Invitations</dt><dd>Not loaded</dd></div><div><dt>Restrictions</dt><dd>Not loaded</dd></div><div><dt>Credential version</dt><dd>{projection.authenticationState.credentialVersion}</dd></div></dl></aside></div>
    </main>
  </AdminPortalShell>;
}
