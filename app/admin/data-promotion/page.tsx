...
export default async function Page({searchParams}:Readonly<{searchParams?:Promise<Readonly<Record<string,string|string[]|undefined>>>}>){
  ...
  const data=await loadAdminDomainSurface({db:getServerFirestore(),key:"data-promotion",scope:access.scope,permissions:Object.freeze(["provider.seed.promote"]),query:query||null,status:status||null});
