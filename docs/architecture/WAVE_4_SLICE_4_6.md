...
The private organization-owned assessment covers fit, eligibility review, capacity, economics, geography and gaps using explicit participant states and bounded notes. One pursuit record exists for `(organization, opportunity reference)` in `watch | pursue | decline`; actor user and membership are evidence, not ownership.

Consequential reads and writes use the canonical server `response.create` authorization boundary. Persistence rechecks membership, restrictions, explicit permission, current open publication/version/digest, issuer separation, released geography and the current organization capability/service-geography digest inside the transaction. Expected versions and stable command fingerprints provide conflict and exact replay behavior. Commands, fit snapshots, events and audit evidence are append-only and directly inaccessible to clients.
