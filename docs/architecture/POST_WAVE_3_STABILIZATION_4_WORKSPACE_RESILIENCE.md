...
The Account organization-profile route starts its optional market-profile, organization-enrichment and participant-map loads concurrently, immediately converts dependency failures into explicit unavailable results, and renders them behind separate Suspense boundaries. Credential, media and additional-location management await only the enrichment projection; the subordinate-location map has its own nested streaming boundary, so map latency or failure cannot withhold the rest of enrichment. Market-profile service-geography labels resolve from independent authoritative geography definitions, preserving human-readable locality names without making the panel depend on the map projection. Organization identity, authorization, completion, location and marker facts remain the essential shell. Optional dependency latency or failure therefore cannot reinterpret an authorized participant as absent or prevent the primary Account workspace from rendering.
