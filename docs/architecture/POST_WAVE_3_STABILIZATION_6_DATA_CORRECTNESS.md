...
Previously stored NAICS descriptors remain visible and are preserved by default when they cannot be represented exactly by the governed release, including older versions and authorized imports. Preservation copies only the server-side stored record; a client cannot manufacture historical provenance. Removing those descriptors or replacing them with a current governed selection requires an explicit participant choice.
