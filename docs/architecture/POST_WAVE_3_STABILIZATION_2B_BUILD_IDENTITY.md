...
`RFXCHANGE_BUILD_SHA` is the canonical build-time input. GitHub Actions supplies `${{ github.sha }}` to the production build. `next.config.ts` accepts only a complete 40-character hexadecimal Git SHA, lowercases it, embeds it into the compiled application environment, and uses the same value as Next.js `BUILD_ID`.

`GITHUB_SHA` is a build-time fallback for environments that already expose GitHub's commit identity. Invalid, missing, abbreviated, or non-hexadecimal values are not presented as a valid RFxchange build identity.
...
Production CI binds `RFXCHANGE_BUILD_SHA` to `${{ github.sha }}` for `npm run build`. After the build, CI requires `.next/BUILD_ID` to equal that exact GitHub SHA. Architecture coverage additionally proves the validated identity source and both visible projections remain connected to the same contract.
