...
PR #270 is merged as `953abdbbe5731f162800f5b96fefb8ff1e39ce0d`; its main CI run [34643511269](https://github.com/AccelAnalysis/RFxchange/actions/runs/34643511269) passed. Local validation includes the full repository gate and the independent application's production HTTP checks. The `rfxchange-admin` backend is created in `us-east4`, linked to the existing Admin Web App, with root `apps/admin`, branch `main`, automatic rollouts disabled and the exact source SHA override set. Deployment and hosted authenticated operation remain pending. Existing root Admin routes stay available until cutover checks pass.
