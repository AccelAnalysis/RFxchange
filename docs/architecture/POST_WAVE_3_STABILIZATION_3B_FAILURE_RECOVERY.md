...
Network-education and atomic referral persistence exceptions are no longer rewritten as ordinary domain conflicts. They propagate to the API boundary as dependency failures, while genuine version, state, validation, and command-identity conflicts keep their domain classifications. Acquisition-context binding now uses a typed domain error for a missing context, wrong browser secret, expiry, or cross-journey reuse. Those permanently invalid contexts are rejected and their cookie is removed; an unclassified bind outage during otherwise successful sign-in is reported as `unavailable` and preserves the cookie for retry. Orientation likewise distinguishes typed participant-scope and journey-state conflicts from repository or provider outages, preserving bounded 403/409 semantics without exception-message projection.
