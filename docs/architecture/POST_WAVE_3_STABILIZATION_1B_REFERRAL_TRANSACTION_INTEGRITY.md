...
The participant snapshot derives a typed `delivery-outcome-unknown` notification state from the durable claim. The Referral and Resource Network workspaces render “Delivery outcome unknown — support reconciliation is required before another attempt” in every supported locale and do not offer Retry for that state. The projection exposes no provider response, claim identifier, or other private operational evidence.
