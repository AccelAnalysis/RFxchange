...
Automatic approval review rejected opening the production Google Cloud Secret Manager console, identifying it as a privileged configuration boundary requiring specific authorization. The existing Firebase CLI session was unauthenticated. No alternate credential path was used. Production secret inspection/configuration and subsequent release therefore remain blocked pending that access authorization.
