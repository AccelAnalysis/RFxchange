...
The owner supplied the SMS/MMS sender `+18337391819` on September 12; it is now the Exchange runtime sender configuration. The owner reports having a Telnyx API key and public key and has since created the API-key secret version recorded below. The public key and messaging-profile ID have not yet been supplied to this worker. The API key remains a Secret Manager runtime binding. The messaging profile assignment, public-key configuration and toll-free verification status still require provider-side inspection. This packet dispatches text-only SMS lifecycle templates; an MMS attachment workflow is not claimed complete merely because the number supports SMS/MMS.
...
The owner has now reported creating `TELNYX_API_KEY` version `1` in the existing `rfxchange` project. Exchange `apphosting.yaml` pins that version with runtime-only availability. This is a source configuration reference, not evidence that backend access was granted, the secret value was inspected, or a production rollout occurred. Production sending remains disabled. The authenticated Firebase console confirmed the existing `rfxchange` backend in `us-east4`, its retained cutover release and only the existing build-SHA console override. Its embedded Cloud Shell also returned `Site Unavailable`, so the secret access grant has not been executed from this session.

Before rolling out that binding, grant the existing Exchange App Hosting backend access to this one secret from an authenticated Firebase CLI (or Google Cloud Shell):
...
This command grants access; it does not print or replace the API key and does not deploy an application. The backend is `projects/rfxchange/locations/us-east4/backends/rfxchange`, as established by the existing cutover record. Confirm the grant before deployment and inspect the rollout's resolved configuration because console variables can override `apphosting.yaml`. The Telnyx public key, messaging-profile ID/number assignment, worker-secret binding, provider acceptance and explicit send gates remain outstanding. Do not add placeholder credentials or bind this key to Marketing, Admin or browser bundles.
