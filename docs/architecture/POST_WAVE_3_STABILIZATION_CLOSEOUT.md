...
Status: independently executable Stabilizations 1–7 closed on 2026-08-10; Stabilization 2C remains externally blocked.

This ledger reconciles the canonical [Post-Wave 3 stabilization authority](./POST_WAVE_3_STABILIZATION.md) against merged production code. The implementation baseline before this closeout record and before PR #150 reconciliation is:
...
| Stabilization 2C — same-SHA App Hosting deployment | **Externally blocked only; not complete** | — | — | — | — | — | Rechecked after Stabilization 7. Blaze billing is enabled and project CLI access works, but `firebase apphosting:backends:list --project rfxchange` returns no backend. The [required GitHub-connected backend creation](https://firebase.google.com/docs/app-hosting/get-started) requires an authenticated Firebase Console session and repository connection/installation; the available console browser is signed out and no authenticated external browser is connected. [Officially documented automatic variables](https://firebase.google.com/docs/app-hosting/configure#automatically-populated-environment-variables) do not include an immutable Git commit SHA, so `RFXCHANGE_BUILD_SHA` must not be guessed or statically drifted before the backend/build environment can be inspected. |
...
- Stabilization 1 has no remaining implementation criterion: profile upload parsing is bounded before copying, create-and-send persistence is atomic and retry-stable, and provider delivery is coordinated through durable authoritative claims rather than a check-then-send race.
- Stabilization 2A and 2B are complete. Only 2C remains open, isolated to production App Hosting provisioning, trustworthy commit injection and live rollout evidence.
...
## Isolated App Hosting prerequisite

The remaining owner-controlled sequence is:

1. sign in to the Firebase Console for project `rfxchange`;
2. create a Firebase App Hosting backend connected to `AccelAnalysis/RFxchange`, root `/`, live branch `main`, with automatic rollout policy intentionally selected;
3. authorize or install the Firebase GitHub integration for that repository if prompted;
4. inspect the first GitHub-backed Cloud Build environment and establish a trustworthy full source-commit input for `RFXCHANGE_BUILD_SHA` without relying on an undocumented variable;
5. roll out one exact merged `main` commit and capture backend/build/rollout identity, hosted URL, live full and short SHA, authenticated Account SHA where practical, timestamp and rollback target.

No local-source backend, classic Hosting deployment, Firebase rollout identifier, abbreviated SHA, secret, login token or service-account JSON substitutes for that evidence. This blocker does not alter the documentation-only interaction decisions in PR #150 and leaves no unmerged implementation branch that can change PR #150's three documents.

## Governance closeout

- No Feature ID was added, removed, renamed, reordered or newly completed.
...
- RFx Core/Wave 4 implementation was not begun.
- Exchange-shell convergence and the persistent four-lens runtime were not begun.
- Intelligence/Locations runtime work was not begun.
- Dark Appearance, Presentation Mode, production sound and haptics were not implemented.

PR #150 may proceed after this ledger merges because Stabilization 2C is precisely isolated to external deployment control and does not change its interaction-architecture authority.
