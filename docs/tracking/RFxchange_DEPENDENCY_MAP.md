...
**Next-authority result:** after Slice 4.4 merges and post-merge validation is green, recalculate from merged `main` and define a documentation-only Slice 4.5 authority for `DSC-004`, `DSC-005`, `DSC-006`, `DSC-007` and `DSC-008`. No discovery/manage runtime or B6c expression begins before its applicable reviewed authority.
