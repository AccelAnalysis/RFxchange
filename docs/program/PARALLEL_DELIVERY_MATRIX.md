...
The permanent order is exactly `Opportunities/RFx | Resources | Intelligence | Referrals`. Runtime availability remains unchanged by the governance installation and operational activation. All four continue through their existing server-authorized runtimes; Opportunities/RFx uses the real Slice 4.5 `/opportunities` discovery runtime. Availability never broadens record or action authority.
...
The five active packets share activation epoch `initial-operational-2026-08-12`. Their immutable activation base is the authority merge SHA `e2e2db0d199f1d10e54c597c581dcf9d8a36dbed`; candidate branches may start from that SHA or a descendant current-main SHA while retaining that activation provenance.
