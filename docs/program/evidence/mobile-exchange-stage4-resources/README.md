...
- `npm run check` under Node 24.18.0: passing; validation, Functions build and 43/43 tests, 788/788 architecture tests, typecheck, lint (zero errors; repository-baseline warnings only) and the production build all passed, including the bounded `/resources/view/[kind]/[id]` fallback route.
