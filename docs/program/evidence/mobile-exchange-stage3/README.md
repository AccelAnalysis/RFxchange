...
- Result sets distinguish loading, ready, empty, unavailable, restricted, and error states. Non-ready states cannot retain protected cards, result-set IDs, record/cluster/area/relationship map projections, or unpaired record points.
- Card, map, keyboard, and deep-link detail entry converge on one subject. Every entry remains opening and does not become an authorized detail until the server returns the exact non-null identity and canonical detail destination. Close restores the valid query, geography scope, camera/bounds, selection, domain-revalidated layers, cursor/page/index, independent list/sheet positions, snap point, lens-bounded origin route, and focus key; scope or authority changes discard the stale snapshot.
