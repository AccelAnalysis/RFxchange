...
const admin = resolveAuthorityContextFromAdminRolePreset(
  "admin-support",
...
function user(id, email) {
  return createUserIdentity({
    id, name: id, primaryEmail: email, loginProvider: "firebase",
    loginSubject: `firebase-${id}`, now: NOW,
  });
}
...
function harness() {
  ...
  const sponsor = user("owner", "owner@example.com");
  const target = user("user-1", "user1@example.com");
  const sponsorMembership = createOrganizationMembership(sponsor, organization, { id: "membership-owner", now: NOW });
  const membership = createOrganizationMembership(target, organization, { id: "membership-1", now: NOW });
  const otherMembership = createOrganizationMembership(target, otherOrganization, { id: "membership-2", now: NOW });
  ...
  const authorizations = new Map([
    [membership.id, authorization], [otherMembership.id, otherAuthorization], [sponsorMembership.id, sponsorAuthorization],
  ]);
  ...
  const service = new ControlledUserAdministrationService({
    ...
    roleBundles: { async getByKey() { return null; }, async listAll() { return []; }, async save() {} },
    unitOfWork: {
      async commit(input) {
        commits.push(input);
  ...
  return {
    organization, otherOrganization, sponsor, target, sponsorMembership, membership, otherMembership,
...
function context(extra = {}) {
  return {
    ...
    securityContext: {
      authenticationSubject: "firebase-admin-support", sessionId: "session-1", provider: "firebase",
...
test("ADM-068 invite and resend are explicit organization-scoped audited actions", async () => {
  const h = harness();
  const invitation = await h.service.invite({
    context: context({ auditEventId: "audit-invite" }), organization: h.organization,
    sponsor: h.sponsor, sponsorMembership: h.sponsorMembership, sponsorAuthorization: h.sponsorAuthorization,
    invitationId: "invitation-1", inviteeEmail: "new.user@example.com", rolePresetKey: "viewer",
  });
  ...
  const resent = await h.service.resendInvitation({
    ...
    organization: h.organization, invitationId: invitation.id,
  });
...
test("ADM-068 scope, expiry and grant conditions fail closed; GLOBAL can cover the organization", async () => {
  const wrong = harness();
  await assert.rejects(() => wrong.service.assignPermission({
    context: context({ grants: [adminGrant("ORGANIZATION:org-b")] }), organization: wrong.organization,
    user: wrong.target, membershipId: wrong.membership.id, permission: "response.create",
  }), /scope-not-satisfied/);

  const expired = harness();
  await assert.rejects(() => expired.service.assignPermission({
    context: context({ grants: [adminGrant("ORGANIZATION:org-a", { expiresAt: "2026-07-30T19:00:00.000Z" })] }),
    organization: expired.organization, user: expired.target, membershipId: expired.membership.id,
    permission: "response.create",
  }), /grant-expired/);

  const conditioned = harness();
  const conditionalGrant = adminGrant("ORGANIZATION:org-a", { id: "grant-conditioned", conditionKeys: ["case-approved"] });
  await assert.rejects(() => conditioned.service.assignPermission({
    context: context({ grants: [conditionalGrant] }), organization: conditioned.organization,
    user: conditioned.target, membershipId: conditioned.membership.id, permission: "response.create",
  }), /conditions-not-satisfied/);
  const allowed = await conditioned.service.assignPermission({
    context: context({ grants: [conditionalGrant], satisfiedGrantConditionKeys: ["case-approved"] }),
    organization: conditioned.organization, user: conditioned.target, membershipId: conditioned.membership.id,
    permission: "response.create",
  });
  assert.deepEqual(allowed.permissions, ["response.create"]);

  const global = harness();
  assert.deepEqual((await global.service.assignPermission({
    context: context({ grants: [adminGrant("GLOBAL")] }), organization: global.organization,
    user: global.target, membershipId: global.membership.id, permission: "response.create",
...
test("ADM-068 organization removal preserves the no-orphan invariant", async () => {
  ...
  const removed = await h.service.removeFromOrganization({
    context: context({ auditEventId: "audit-remove" }), organization: h.organization,
    user: h.target, membershipId: h.membership.id,
  });
  ...
  await assert.rejects(() => orphan.service.removeFromOrganization({
    context: context({ auditEventId: "audit-orphan" }), organization: orphan.organization,
    user: orphan.target, membershipId: orphan.membership.id,
  }), /orphan an active user/);
});

test("ADM-068 suspension and restore reuse the membership restriction state machine", async () => {
  const h = harness();
  const suspended = await h.service.suspend({
    context: context({ auditEventId: "audit-suspend" }), organization: h.organization,
    user: h.target, membershipId: h.membership.id, restrictionId: "restriction-1",
  });
  ...
  const restored = await h.service.restore({
    ...
    organization: h.organization, user: h.target, membershipId: h.membership.id,
  });
...
test("ADM-068 approved access reset requires a distinct approval", async () => {
  const h = harness();
  await assert.rejects(() => h.service.resetAccess({
    context: context({ auditEventId: "audit-reset-denied" }), organization: h.organization,
    user: h.target, membershipId: h.membership.id, approvedRoleBundleKey: "viewer",
  }), /distinct administrative approval/);
  const reset = await h.service.resetAccess({
    context: context({ auditEventId: "audit-reset", approvalReferences: [{ approvalId: "approval-1", approverAdministratorId: "admin-secondary" }] }),
    organization: h.organization, user: h.target, membershipId: h.membership.id,
...
test("ADM-068 role transfer and permission assignment/revocation are separately audited", async () => {
  const h = harness();
  const role = await h.service.transferRole({
    context: context({ auditEventId: "audit-role" }), organization: h.organization,
    user: h.target, membershipId: h.membership.id, roleBundleKey: "billing-manager",
  });
  assert.equal(role.roleKey, "billing-manager");
  const assigned = await h.service.assignPermission({
    context: context({ auditEventId: "audit-assign", now: "2026-07-30T20:01:00.000Z" }), organization: h.organization,
    user: h.target, membershipId: h.membership.id, permission: "response.create",
  });
  assert.deepEqual(assigned.permissions, ["billing.manage", "response.create"]);
  assert.deepEqual((await h.service.revokePermission({
    context: context({ auditEventId: "audit-revoke", now: "2026-07-30T20:02:00.000Z" }), organization: h.organization,
    user: h.target, membershipId: h.membership.id, permission: "billing.manage",
  })).permissions, ["response.create"]);
...
test("ADM-068 sensitive mutations require reauthentication context", async () => {
  ...
  await assert.rejects(() => h.service.assignPermission({
    context: {
      ...
      securityContext: { authenticationSubject: "firebase-admin-support", sessionId: "session-1", provider: "firebase", reauthenticatedAt: "" },
    },
    organization: h.organization, user: h.target, membershipId: h.membership.id, permission: "response.create",
