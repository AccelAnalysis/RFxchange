...
test("EDU-009 defines every semantic destination without persisting route authority", () => {
  ...
  for (const intent of FIRST_VALUE_INTENTS) {
    ...
    assert.ok(destination.route === null || destination.route === "/geography/canvas" || destination.route === "/referrals?intent=manage");
  }
  assert.equal(FIRST_VALUE_DESTINATIONS["send-receive-referral"].route, "/referrals?intent=manage");
