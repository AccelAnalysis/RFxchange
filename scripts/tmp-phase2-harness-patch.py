...
start_marker = "async function clickLens(cdp, id, expectedPath, options = {}) {"
end_marker = "\n\nasync function clickUtility"
if source.count(start_marker) != 1:
    raise SystemExit(f"Expected one clickLens start marker, found {source.count(start_marker)}.")
start = source.index(start_marker)
end = source.index(end_marker, start)
replacement = r'''let exchangeRoomReopenEvidenceCaptured = false;
