...
assert.match(publication, /governedResponderOpportunityProjection/);
assert.match(publication, /stableDigest\(projection\.payload\)/);
...
assert.match(service, /governedResponderOpportunityProjection/);
assert.match(service, /expectedFitSnapshotId/);
assert.match(service, /reconfirmedStaleInputs/);
assert.match(service, /pursuitIsStale/);
...
assert.match(repository, /governedResponderOpportunityProjection/);
...
assert.match(route, /reconfirmedStaleInputs: body\.reconfirmedStaleInputs === true/);
...
assert.match(workspace, /fieldset key=\{key\} disabled=\{!workspace\.canManage\}/);
assert.match(workspace, /pursuitReadOnly/);
assert.match(workspace, /staleInputsConfirmed/);
assert.match(workspace, /reconfirmedStaleInputs: workspace\.stale \? staleInputsConfirmed : false/);
assert.match(workspace, /workspace\.stale && !staleInputsConfirmed/);
...
console.log("Wave 4 Slice 4.6 deterministic fit, gap, assessment, organization pursuit, and Four-Lens reconciliation guardrails validated.");
