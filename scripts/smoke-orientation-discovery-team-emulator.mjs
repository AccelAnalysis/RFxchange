...
try {
  ...
  await service.completeStep(scope, "three-organization-scenario");
  await service.completeStep(scope, "opportunity-issuance");
  await service.completeStep(scope, "capability-match");
  const progressed = await service.completeStep(scope, "gap-and-teammate-discovery");
  assert.equal((await repository.getById(started.id))?.completedThroughStep, 4);
  assert.equal(progressed.revision, 5);
  await assert.rejects(service.completeStep(scope, "teammate-invitation"), /not enabled/);
  ...
  console.log("EDU-001-004 orientation emulator smoke passed.");
} finally {
